...
//echo $campaigns->found_posts;
    ...
    if ($filters == 1 && !empty($terms))
    {
        ...
        {
            echo '<li data-option-value=".campaign_category-' . $term->slug . '"><a href="#"><span>' . $term->name . '</span></a> <h5>term_id ' . $term->term_id . '</h5></li>';
        }
        echo '</ul>
                </div>';
    }
    ...
    if ($carousel == 'carousel')
    {
        ...
        <div class="row">
        <ul class="owl-carousel carousel-fw" id="causes-slider" data-columns="<?php echo esc_attr($columns); ?>" data-autoplay="" data-pagination="no" data-arrows="yes" data-single-item="no" data-items-desktop="4" data-items-desktop-small="3" data-items-tablet="2" data-items-mobile="1" <?php if (is_rtl())
    { ?>data-rtl="rtl"<?php } else
    { ?> data-rtl="ltr" <?php } ?>>
    ...
    {
        echo '<ul class="sort-destination isotope gallery-items" data-sort-id="gallery">';
    }
    $counter = 1;

//    query_posts('cat=54&showposts=3');
